...
const validateEmail = (email) => {
  return /^\w+\.\w+@\w+\.\w{3}|^\w+@\w+\.\w{3}\b/mg.test(email);
};
// 
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
...
const findTagNames = elements => {
  return elements.map(string => string.match(/<(\/[a-z]+\d?)>/g)).reduce((acc, currVal) => acc.concat(currVal.map(el => el.slice(1,(el.length-1)))), []);
};
