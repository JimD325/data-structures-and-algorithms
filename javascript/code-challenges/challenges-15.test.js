...
const sortBy = (property, arr) => {
  return arr.sort((pv, cv)=> {
    if (pv[property]<cv[property]){
      return -1
    } else if (pv[property]>cv[property]){
      return 1;
    }
    else {
      return 0;
...
// because you want to be able to sort by either name OR price, you need to use a variable. Variables are NOT accessible via dot notation, you need to use [] notation to use a variable
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5 
