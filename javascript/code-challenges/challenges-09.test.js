...
const { default: DropdownMenu } = require("react-bootstrap/esm/DropdownMenu");
...
const updateNumbers = (obj) => {
  let arr = [];
  Object.entries(obj).forEach((entry,idx) => arr.push (Object.keys(obj)[idx]+ ': '+Object.values(obj)[idx]));
  return arr;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(entry => {
    if (entry.name === character){
      Object.keys(entry).forEach((value, idx)=>{
        if (value === 'children'){
          children = Object.values(entry)[idx].length;
        }
      });
    }
  });
