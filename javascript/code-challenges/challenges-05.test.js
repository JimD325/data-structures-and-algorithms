...
// The final index of the array is always equal to one less than its length. so while using the reduce method, enter in a dummy value for pv and cv, since in this case it doesnt matter what they are doing. Then return the index +1, which after the reduce method has been completely finished will be equal to the array length.
...
// in English: reduce the array, where PV is equal to an empty array, and you push into this empty array the names from each of the objects using cv.name.
